...
spark = SparkSession.builder.appName("SparkSQL_Q2_parquet").config("spark.ui.port", "4041").getOrCreate()
...
start = time.time()
# execute query
result.coalesce(1).write.mode('overwrite').csv('hdfs://master:9000/outputs/q2_sql_result_parquet.csv')
end = time.time()
...
with open('time_outputs.txt', 'a', newline='') as csvfile:
    ...
    data = ['Query 2', 'SQL API (parquet)', total_time]
