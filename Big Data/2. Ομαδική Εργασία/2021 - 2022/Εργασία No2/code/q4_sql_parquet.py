...
spark = SparkSession.builder.appName("SparkSQL_Q4_sql_parquet").config("spark.ui.port", "4041").getOrCreate()
...
start = time.time()
# execute query
result.coalesce(1).write.mode('overwrite').csv('hdfs://master:9000/outputs/q4_sql_result_parquet.csv')
end = time.time()
...
with open('time_outputs.txt', 'a', newline='') as csvfile:
    ...
    data = ['Query 4', 'SQL API (parquet)', total_time]
