...
# This script was created for testing the running time of different FIM implementation on
...
echo RUNNING TIMES \(USER + SYSTEM\) >>$result_file
...
do
	...
	do
		...
		do
			/usr/bin/time -f "%U + %S" -o temp_time.txt $program ${datadir}${database[$index]} output.txt $minsupp;
			bc < temp_time.txt >> $result_file;
		done;
